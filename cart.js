...
const recommendationKey = "savorly-recommendation-items";
...
const catalog = { ...baseCatalog, ...readRecommendationCatalog() };

let cart = readCart();
let fulfillment = "";
let promoApplied = false;

function readRecommendationCatalog() {
  try {
    const saved = JSON.parse(localStorage.getItem(recommendationKey)) || {};
...
function renderCart() {
  ...
  itemsContainer.innerHTML = entries.map(([id, quantity]) => {
    ...
    const safeArt = ["art-tomatoes", "art-salmon", "art-pasta", "art-cake", "art-recommendation"].includes(item.art) ? item.art : "art-recommendation";
    return `
      <article class="cart-item" data-cart-item="${id}">
        <div class="cart-item-art ${safeArt}" aria-hidden="true"></div>
        <div class="cart-item-info">
          <h3>${safeName}</h3>
          <p>${safeDescription}</p>
          <div class="item-controls">
            <div class="quantity-control" aria-label="Quantity for ${safeName}">
              <button type="button" data-action="decrease" aria-label="Decrease ${safeName} quantity">−</button>
              <span>${quantity}</span>
              <button type="button" data-action="increase" aria-label="Increase ${safeName} quantity">+</button>
            </div>
...
const takeoutContact = document.querySelector("#takeout-contact");
const googleOrderForm = document.querySelector("#google-order-form");
fulfillmentInputs.forEach((input) => {
  input.addEventListener("change", () => {
    ...
    takeoutContact.hidden = fulfillment !== "takeout";
    renderSummary();
  });
});
...
document.querySelector("#checkout-button").addEventListener("click", () => {
  ...
  if (fulfillment === "takeout" && !googleOrderForm.reportValidity()) {
    takeoutContact.scrollIntoView({ behavior: "smooth", block: "center" });
    return;
  }
  if (fulfillment === "takeout") {
    googleOrderForm.submit();
    status.textContent = `Your takeout details were sent to the restaurant form. Your order total is ${money(calculateTotals().total)}.`;
  } else {
    status.textContent = `Great choice! Your dine-in order total is ${money(calculateTotals().total)}. Checkout is ready for the next build.`;
  }
  status.hidden = false;
  status.focus();
});
